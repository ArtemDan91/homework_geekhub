...
class Cart:
    ...
    def get_cart_product_data(self, product_id):
        product = get_object_or_404(ScrapedProduct, product_id=product_id)

        if product_id in self.cart:
            product_quantity = self.cart[product_id]
            return {
                'product_id': product_id,
                'product_description_name': product.product_description_name,
                'sell_price': product.sell_price,
                'quantity': product_quantity,
                'products_total_cost': round(product.sell_price * product_quantity, 2)
            }
    ...
    def get_all_cart_products(self):
        ...
        for product_id, quantity in self.cart.items():
            product = get_object_or_404(ScrapedProduct, product_id=product_id)
            products_total_cost = round(product.sell_price * quantity,
                                        2)
