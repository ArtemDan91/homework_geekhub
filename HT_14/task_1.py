...
from HT_12.atm_cash_out_algorithm import cash_out_with_min_bills
from HT_14.currency_rates import get_current_exchange_rates
...
class UnavailableDenominationsError(Exception):
    pass

class DataBaseManager:
    
    def __init__(self, database_path=parent_dir / 'HT_12' / 'atm_database.db'):
...
class ATMOperations:
    ...
    def cash_out(self, current_user):
        try:
            ...
            if self.user_manager.conn:
                ...
                atm_denominations = self.user_manager.cur.execute('SELECT denomination, count FROM atm_banknotes WHERE count > 0').fetchall()
                atm_available_denominations = [item[0] for item in atm_denominations]
                count_of_denominations = [item[1] for item in atm_denominations]
             
                banknotes_output_dict = cash_out_with_min_bills(atm_available_denominations, amount, count_of_denominations)
                
                if not banknotes_output_dict:
                    raise UnavailableDenominationsError(f"Необхідну суму {amount} грн. не можливо видати доступними у банкоматі номіналами")
                else:
                    self.user_manager.cur.execute('UPDATE user_balance SET balance = balance - ? WHERE username = ?', (amount, current_user))
                    self.user_manager.cur.execute('UPDATE atm SET balance = balance - ?', (amount,))
                    print(f"Знято {amount} грн. з рахунку {current_user}.")
                    print("Банкоматом видані наступні купюри:")
                    for banknot, banknot_count in banknotes_output_dict.items():
                        self.user_manager.cur.execute('UPDATE atm_banknotes SET count = count - ? WHERE denomination = ?', (banknot_count, banknot))
                        self.user_manager.conn.commit()
                        print(f"Номінал {banknot} грн. - {banknot_count} шт.")

                    self.user_manager.conn.commit()
                self.update_transactions_file(current_user, 'cash_out', amount)

        except (NegativeInputAmountError, NegativeAtmBalanceError, NegativeUserBalanceError, UnavailableDenominationsError) as error:
            print(error)
        except sqlite3.Error as e:
            print(f"Помилка при роботі з SQLite: {e}")
        finally:
    ...
    def view_current_exchange_rates(self):
        try:
            ...
            if exchange_rates:
                ...
                for currency, rates in exchange_rates.items():
                    print(f"{currency}: курс купівлі - {rates['buy']}, курс продажу - {rates['sell']}")
    ...
    def atm_operations(self, current_user):
        user_operations = {
            ...
            '5': {'title': 'Переглянути поточний курс валют', 'operation': self.view_current_exchange_rates},
            '6': {'title': 'Вихід', 'operation': self.exit_operation},
        }
        cashier_operations = {
            '7': {'title': 'Переглянути баланс банкомату', 'operation': self.display_atm_balance},
            '8': {'title': 'Переглянути залишок купюр в банкоматі', 'operation': self.view_banknotes_balance},
            '9': {'title': 'Змінити кількість купюр', 'operation': self.update_banknotes_quantity},
        }
        ...
        while True:
            try:
                ...
                if choice in operations:
                    ...
                    if operation not in (self.update_banknotes_quantity, self.view_banknotes_balance, self.display_atm_balance, self.view_current_exchange_rates):
                        operation(current_user)
                    else:
                        operation()

                    if choice == '6' or not self.ask_user_to_continue("Бажаєте продовжити проводити операції з банкоматом? (1 - так / 0 - ні): "):
                        break
